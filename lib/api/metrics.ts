...
export async function validateMetricFormula(
  ...
  if (!response.ok) {
    throw new Error("Failed to validate metric formula.");
  }
...
export async function updateMetric(
  ...
  if (!response.ok) {
    ...
    throw new Error(text || "Failed to update metric");
  }
...
export async function interpretMetric(
  ...
  const response = await fetch(
    `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/metrics/interpret`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
